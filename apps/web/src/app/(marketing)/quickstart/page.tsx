...
const STEPS: Step[] = [
  {
    ...
    description:
      "No install required — run directly with bunx or npx. Or install globally if you prefer `ov` always on your PATH.",
    terminalTitle: "install",
    lines: [
      { type: "comment", text: "# zero-install — run directly (recommended)" },
      { type: "prompt", text: "bunx @open-vault/cli onboard" },
      { type: "blank", text: "" },
      { type: "comment", text: "# or with npx" },
      { type: "prompt", text: "npx @open-vault/cli onboard" },
      { type: "blank", text: "" },
      { type: "comment", text: "# install globally for `ov` shorthand" },
    ...
    note: "Requires Bun ≥ 1.0 or Node ≥ 20. No other dependencies. The `onboard` command guides you through init + login in one step.",
  },
  {
    id: "init",
    number: 2,
  ...
  },
  {
    id: "mcp",
    number: 10,
    title: "(Optional) Connect Claude via MCP",
    description:
      "Give Claude zero-trust read access to your secrets using the @open-vault/mcp server. Decryption is local — plaintext never leaves your machine. Works with Claude Desktop and any MCP-compatible AI tool.",
    terminalTitle: "MCP server",
    lines: [
      { type: "comment", text: "# claude_desktop_config.json — add this entry:" },
      { type: "output", text: '{' },
      { type: "output", text: '  "mcpServers": {' },
      { type: "output", text: '    "open-vault": {' },
      { type: "output", text: '      "command": "bunx",' },
      { type: "output", text: '      "args": ["@open-vault/mcp"]' },
      { type: "output", text: '    }' },
      { type: "output", text: '  }' },
      { type: "output", text: '}' },
      { type: "blank", text: "" },
      { type: "comment", text: "# install Claude Code skill" },
      { type: "prompt", text: "cp packages/mcp/open-vault.skill.md ~/.claude/skills/open-vault.md" },
      { type: "blank", text: "" },
      { type: "comment", text: "# or run the server directly (zero-install)" },
      { type: "prompt", text: "bunx @open-vault/mcp" },
    ],
    note: "The MCP server exposes three read-only tools: list_projects, list_secrets, and get_secret. Secrets are decrypted client-side with your SSH key — the server never sees plaintext. npx @open-vault/mcp works if you prefer npm.",
  },
];
